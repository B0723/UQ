...
def get_overall_log_likelihoods(list_of_results):
    ...
    list_of_keys = ['neg_log_likelihoods', 'average_neg_log_likelihoods', 'sequence_embeddings',\
                    'new_sequence_embeddings','pointwise_mutual_information', 'average_neg_log_likelihood_of_most_likely_gen',\
                    'neg_log_likelihood_of_most_likely_gen','average_neg_log_likelihood_of_second_most_likely_gen','semantic_set_ids',\
                    'new_average_neg_log_likelihoods']
    ...
    for key in list_of_keys:                                                                                                                                                                                                                                                                                                                                                                  
        ...
        for model_size, result in list_of_results:
            ...
            for sample in result:
                ...
                if key == 'new_average_neg_log_likelihoods':
                    average_neg_log_likelihoods = torch.where(torch.isnan(average_neg_log_likelihoods),torch.tensor(0.0),average_neg_log_likelihoods)
                if key == 'new_sequence_embeddings':
                    average_neg_log_likelihoods = torch.stack(sample[key])
        ...
        if key != 'sequence_embeddings' or key != 'new_sequence_embeddings':
...
def get_predictive_entropy_over_concepts(log_likelihoods, semantic_set_ids):
    ...
    for row_index in range(mean_across_models.shape[0]):
        ...
        row = mean_across_models[row_index].to(semantic_set_ids_row.device)
...
new_predictive_entropy_over_concepts = get_predictive_entropy_over_concepts(-overall_results['new_average_neg_log_likelihoods'],
                                                                        overall_results['semantic_set_ids'])
...
overall_results['new_predictive_entropy_over_concepts'] = new_predictive_entropy_over_concepts
